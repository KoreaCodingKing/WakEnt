...
} from "../../components/isedol/Members/Utils";
...
const ContentsMain: NextPage = () => {
  ...
  const [choosenContentName, setChoosenContentName] =
    useState<ContentName | null>();
    ...
    useCallback(() => {
      setTimeout(() => {
        ...
        setCurrentContents(
          contents.slice(
            0,
            contents.length >= (page + 2) * numberOfContentsByPage
              ? (page + 2) * numberOfContentsByPage
              : contents.length
          )
        );
      }, 200);
  ...
  const filterContents = useCallback(
    (game: Game | string, contentName?: ContentName) => {
      ...
      if (game && !contentName) {
        setChoosenContentName(null);
        setContents(
          contentsList.filter((content: Content) => content.game === game)
        );
        return;
      }
      ...
      setContents(
        contentsList.filter(
          (content: Content) =>
            content.game === game && content.content === contentName
        )
      );
    },
    []
